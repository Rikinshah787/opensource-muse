...
export function DelegateSheet() {
  ...
  return (
    <Sheet
      ...
      subtitle="OpenSource Muse saves a plan and keeps working on the server."
      onClose={close}
    >
    ...
    <View style={{ paddingVertical: 18, borderBottomWidth: 1, borderBottomColor: colors.line }}>
      ...
      {expanded && (
        <View style={{ gap: 15, marginTop: 18, paddingLeft: 48 }}>
          ...
          {editing && (
            <Field
              label="What should OpenSource Muse do?"
              value={prompt}
              onChangeText={setPrompt}
              multiline
            />
    ...
    <Card>
      ...
      <Text style={[s.small, { marginBottom: 14 }]}>
        {sample
          ...
          : "OpenSource Muse checks this public page on the server and saves meaningful changes in Notifications."}
      </Text>
      <ErrorNotice error={error} />
      <Button
        primary
...
export function AppsScreen() {
  ...
  const [name, setName] = useState(data?.identity.name || "OpenSource Muse");
