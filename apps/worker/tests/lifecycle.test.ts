...
test("real Chromium cleans failed profiles and restores a saved UUID after worker restart", {
  ...
  const dataDir = await mkdtemp(join(tmpdir(), "OpenSource Muse-browser-lifecycle-"));
  ...
  try {
    ...
    try {
      ...
      await page.evaluate(() => localStorage.setItem("OpenSource Muse-profile-test", "retained"));
    } finally {
      await context.close();
    }
    await browser.close();
      ...
      state.origins
        ...
        ?.localStorage.some(
          (item: { name: string; value: string }) =>
            item.name === "OpenSource Muse-profile-test" && item.value === "retained",
        ),
    );
  } finally {
    await browser.close();
