# OpenSource Muse browser worker

An independent Node/Playwright service for the OpenSource Muse server. There is no OpenBot dependency. The server owns app authentication and user ownership; this worker accepts requests only from a trusted server holding `WORKER_TOKEN`.
...
The local worker binds to `127.0.0.1:8790` and stores profiles in `.opensource-muse/browser-profiles` by default. Docker sets `WORKER_HOST=0.0.0.0` inside its container; Compose publishes only the loopback host port. `WORKER_DATA_DIR` selects another private profile directory.
