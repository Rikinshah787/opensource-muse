...
export function computerIdentity(config: Config, owner: string) {
  ...
  const name = `OpenSource Muse-${deployment}-${ownerHash}`;
  return {
    container: `${name}-computer`,
    volume: `${name}-workspace`,
    labels: {
      "dev.opensource-muse.managed": "computer-v1",
      "dev.opensource-muse.deployment": deployment,
      "dev.opensource-muse.owner": ownerHash,
...
export class ComputerService {
  ...
  private image() {
    const image = this.config.computerImage ?? "opensource-muse-computer:local";
