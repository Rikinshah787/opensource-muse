...
export const demoModel = "openai/OpenSource Muse-browser-demo";
...
function demoMailResponse(request: ChatCompletionRequest, turn: ChatMessage[]): FixtureResponse {
  const read = turnResult(turn, "read_mail_thread", "call_OpenSource Muse_demo_mail_read_");
  if (read) {
    const parsed = z
      .object({
  ...
  const search = turnResult(turn, "search_mail", "call_OpenSource Muse_demo_mail_search_");
  if (search) {
    const parsed = z
    ...
    return {
      ...
      toolCalls: [
        {
          id: `call_OpenSource Muse_demo_mail_read_${randomUUID()}`,
  ...
  return {
    ...
    toolCalls: [
      {
        id: `call_OpenSource Muse_demo_mail_search_${randomUUID()}`,
        name: "search_mail",
...
export function demoResponse(request: ChatCompletionRequest): FixtureResponse {
  ...
  const result = turn.findLast(
    (message) =>
      ...
      (calls.has(message.tool_call_id) ||
        message.tool_call_id.startsWith("call_OpenSource Muse_demo_browse_")),
  );
  ...
  return {
    ...
    toolCalls: [
      {
        id: `call_OpenSource Muse_demo_browse_${randomUUID()}`,
        name: "browse_web",
...
export function createDemoModel(
  ...
  }).on({ model: "OpenSource Muse-browser-demo" }, demoResponse, {
