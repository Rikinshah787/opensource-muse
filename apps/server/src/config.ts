...
export function readConfig(): Config {
  ...
  const config: Config = {
    ...
    publicUrl,
    dataDir: resolve(process.env.DATA_DIR ?? ".opensource-muse"),
    databaseUrl: process.env.DATABASE_URL,
    accessKey: process.env.OPENSOURCE_MUSE_ACCESS_KEY,
    ...
    computerImage: process.env.COMPUTER_IMAGE ?? "opensource-muse-computer:local",
  ...
  if (
    ...
    throw new Error(
      "Live mode requires OPENSOURCE_MUSE_ACCESS_KEY (24+ characters) and TOKEN_ENCRYPTION_KEY (32-byte base64)",
    );
