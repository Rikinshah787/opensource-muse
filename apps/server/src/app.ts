...
export async function createApp(
  ...
  app.onError((error, c) => {
    ...
    console.error(`[OpenSource Muse] ${error.name}`);
    return c.json(
      {
        error:
  ...
  app.get("/api/google/callback", async (c) => {
    if (c.req.query("error"))
      return c.html("<h1>Google connection cancelled</h1><p>You can return to OpenSource Muse.</p>", 400);
    const state = c.req.query("state"),
      code = c.req.query("code");
    if (!state || !code) throw new AppError("Google callback is incomplete");
    await google.callback(state, code);
    return c.html(
      "<h1>Google is connected</h1><p>Return to OpenSource Muse and refresh your workspace.</p>",
    );
  });
  ...
  app.get("/", (c) =>
    c.json({ name: "OpenSource Muse", app: "http://localhost:8081", health: "/api/health" }),
  );
